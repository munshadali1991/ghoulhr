...
import { emailOrgAdminCredentials } from '@/features/super-admin/api/organizationsApi';
 ...
 * }} props
 */
export function OrgAdminCredentialsDialog({
  open,
  credentials,
  onClose,
  onNotify,
  ...
  const handleSendEmail = async () => {
    if (!credentials?.organizationId || !credentials?.temporaryPassword) return;
    setEmailError('');
    setEmailSuccess('');
    setEmailing(true);
    try {
      const result = await emailOrgAdminCredentials(
    ...
    <Dialog open={open} onClose={handleClose} maxWidth="sm" fullWidth>
      <DialogTitle>
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 1 }}>
          ...
          <Typography variant="h6">Admin Password Regenerated</Typography>
        </Box>
      </DialogTitle>
      ...
      <DialogActions sx={{ px: 3, py: 2, gap: 1, flexWrap: 'wrap' }}>
        ...
        <Button
          ...
          disabled={emailing || !credentials?.temporaryPassword}
        >
